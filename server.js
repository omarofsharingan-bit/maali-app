...
// Helper: try OpenRouter first, fall back to Gemini on any failure
async function groqChat(prompt, maxTokens = 4096) {
  try {
    ...
    console.warn('OpenRouter failed, falling back to Gemini:', orErr.response?.data?.error?.message || orErr.message);
    const res = await axios.post(
